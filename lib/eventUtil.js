...
module.exports = {
  //form an event object from Http request head parameters
  ...
  //add a user to a certain group, this is somtimes an early step before registering a user to an event
  ...
  sendNotifyMailOnEventCreate: function(event, cb) {
    var publish_notify_mail = this.createEventMail(event);
    groupUtil.sendNotifyMailToGroupUsers(event.group, publish_notify_mail, cb);
  },
    ...
    Event.find({
      ...
      if (err) {
        ...
        if (events.length !== 0) {
          ...
          events[0].user.add(user.id);
          events[0].save(function(err, saved) {
              return cb(err, saved);
          });
        } else {
  ...
  //get the user information for callback function if the user is registered to one certain event
    ...
    Event
      ...
      .exec(function(err, events) {
        if (err) {
          ...
          if (events.length !== 0) {
            for (var i = 0; i < events[0].user.length; i++) {
              if (userid === events[0].user[i].id) {
                return cb(null, events[0].user[i]);
              }
            }
            err = 'Could not find user in the event';
            return cb(Error(err));
          } else {
            err = 'The event is not existed';
            return cb(Error(err));
          }
        }
      });
