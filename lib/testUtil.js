...
module.exports = {
  ...
  //Read questions from one existing file, then do callback function
  ...
  //Read questions by giving paperID, then do callback function
  ...
  //Inquire the database to get a paper's question choices by the giving paper ID, then do callback function
  initReport: function(testId, cb){

    var that = this;
    var paperId;

    Paper.find({id: testId}, function(err, founds){
      if(err){
        return cb(err, null);
      }else{
        if(founds.length===0){
  ...
  //Calculate the amount of choices to every question of a test
