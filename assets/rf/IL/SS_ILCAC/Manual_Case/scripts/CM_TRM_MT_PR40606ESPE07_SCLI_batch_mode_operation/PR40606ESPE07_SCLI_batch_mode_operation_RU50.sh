...
resource_prepare()
{
   prepare_ipro_info=`fsclish -c "show networking ipro ipbr-id 1 iface $iface owner $owner ip-address $ip_address" 2>&1`
   ...
   if [ "$prepare_ipbr_info" != "IPBR added successfully." ] 
   then 
...
case_add()
{
   ...
   if [ $err_num -ne 1 ]
   then
      echo "Case fail: PR40606ESPE07_add_IPBR_failure, err_num is $err_num"
      exit
   fi

   if [ "$err_info" != "$ipbr_add_err" ]
   then
      echo "Case fail: PR40606ESPE07_add_IPBR_failure, $err_info"
      exit
   fi

   check_ipro=`fsclish -c "show networking ipro ipbr-id 1 iface $iface owner $owner ip-address $ip_address" 2>&1`
   if [ "$check_ipro" != "IPRO does not exist." ]
   then
      echo "Case fail: PR40606ESPE07_add_IPBR_failure, $check_ipro"
      exit
   fi

   ipbr_info="IPBR with ipbr-id (2001) does not exist."
   check_ipbr=`fsclish -c "show networking ipbr ipbr-id 2001" 2>&1`
   if [ "$check_ipbr" != "$ipbr_info" ]
   then
      echo "Case fail: PR40606ESPE07_add_IPBR_failure, $check_ipbr"
      exit
   fi
...
case_delete()
{
   ipro_delete_err="delete networking ipro ipbr-id 2000 iface $iface owner $owner ip-address ${ip_address},176"
   ...
   if [ $err_num -ne 1 ]
   then 
      echo "Case fail: PR40606ESPE07_add_IPRO_failure, err_num is $err_num"
      exit
   fi

   if [ "$err_info" != "$ipro_delete_err" ]
   then
      echo "Case fail: PR40606ESPE07_add_IPRO_failure, $err_info"
      exit
   fi

   ipbr_info="IPBR with ipbr-id (2000) does not exist."
   check_ipbr=`fsclish -c "show networking ipbr ipbr-id 2000" 2>&1`
   if [ "$check_ipbr" != "$ipbr_info" ]
   then
      echo "Case fail: PR40606ESPE07_delete_IPRO_failure, $check_ipbr"
      exit
   fi

   ipbr_info="IPBR with ipbr-id (2002) does not exist."
   check_ipbr=`fsclish -c "show networking ipbr ipbr-id 2002" 2>&1`
   if [ "$check_ipbr" = "$ipbr_info" ]
   then
      echo "Case fail: PR40606ESPE07_delete_IPRO_failure, $check_ipbr"
      exit
...
case_show()
{
   ...
   then
      echo "Case fail: PR40606ESPE07_show, $fail_exe_num"
      exit
   fi
   ...
   if [ "$check_ipbr" != "$ipbr_info" ]
   then
      echo "Case fail: PR40606ESPE07_show, $check_ipbr"
      exit
   fi
